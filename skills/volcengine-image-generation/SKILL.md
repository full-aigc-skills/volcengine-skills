...
1. 用户明确要使用 Seedream 生成可追溯的图像资产。
2. 已提供或可安全取得必要上下文，需要得到可验证的 `seedream-image-result`。
...
Step 4：生成或执行 `seedream-image-result`，每一步都绑定输入、预期输出与失败条件。
...
- [ ] 目标、环境与授权范围均已写明。
- [ ] `图像、模型、尺寸、提示参数和任务响应可追溯` 已由可复现证据验证。
- [ ] 敏感数据已脱敏，输出中没有完整凭据。
- [ ] 失败与超时路径已覆盖，未出现无限重试。
- [ ] 变更类任务具有备份或回滚说明。
- [ ] 最终结论区分事实、推断和未验证项。
...
2. **成功码不等于业务成功**：必须检查 `图像、模型、尺寸、提示参数和任务响应可追溯`，不能只看命令退出码或 HTTP 200。
